...
router.post('/category/update', async function(req, res, next) {
    var checker = await Category.findOne({ name: req.body.name })
    if (checker == null) {
        await User.updateOne({ _id: req.body.id }, { $set: { name: req.body.name, image: req.body.image } }, ).then(r => {
            Category.find().then(field => {
                res.json({
                    message: 'success',
                    data: field
...
router.post('/category/delete', async function(req, res, next) {
    var checker = await Category.findOne({ name: req.body.name })
    if (checker == null) {
        await User.updateOne({ id: req.body.id }, { $set: { name: req.body.name, image: req.body.image } }, ).then(r => {
            Category.find().then(field => {
                res.json({
                    message: 'success',
                    data: field
