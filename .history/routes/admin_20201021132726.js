...
router.get('/variable/bysubcat', async function(req, res, next) {
    var color = await Variable.find({ type: 'color', subcategory: subcatId }).populate("category subcategory")
    var size = await Variable.find({ type: 'size', subcategory: subcatId }).populate("category subcategory")
    res.json({
        message: 'success',
        color: color,
        size: size
